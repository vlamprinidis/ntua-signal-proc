function hpol = semilogr_polar(varargin)
%POLAR  Polar coordinate plot with logarithmic radius axis (dB).
%   POLAR(THETA, RHO) makes a plot using polar coordinates of
%   the angle THETA, in radians, versus the radius RHO.
%   The radius RHO is plotted in dB, i.e. 10 * log10(RHO).
...
theta(rho < 0) = theta(rho < 0) + pi;
rho = abs(rho);

if min(abs(rho)) < 1e-3
    min_rho_threshold_dB = max(5 .* ceil(2 .* log10(min(rho))), -40);
else
    min_rho_threshold_dB = max(1 ./ 1e2 .* ceil(1e3 .* log10(min(rho))), -40);disp(min_rho_threshold_dB);
end
min_rho_threshold = 10 .^ (min_rho_threshold_dB ./ 10);
theta = theta(rho >= min_rho_threshold);
rho = rho(rho >= min_rho_threshold);

rho = 10 .* log10(rho);
if (min_rho_threshold_dB < 0)
    rho = -min_rho_threshold_dB + rho;
end
...
% draw radial circles
    ...
    for i=(rmin+rinc):rinc:rmax
        ...
        if min_rho_threshold_dB < 0
            ...
            ['  ' num2str(i+min_rho_threshold_dB) 'dB'],'verticalalignment','bottom',...
            'handlevisibility','off','parent',cax)
        else
            text((i+rinc/20)*c82,(i+rinc/20)*s82, ...
            ['  ' num2str(i) 'dB'],'verticalalignment','bottom',...
            'handlevisibility','off','parent',cax)
        end
    end
